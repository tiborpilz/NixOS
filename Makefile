...
deploy_klaus: ## Deploy the klaus nixos configuration to 192.168.1.51 (local adress)
	nix run nixpkgs#nixos-rebuild -- switch --flake .#klaus --target-host root@192.168.1.51 --build-host root@192.168.2.134 --fast

deploy_klaus_dry_run:
	nix run nixpkgs#nixos-rebuild -- dry-run --flake .#klaus --target-host root@192.168.1.51 --build-host root@192.168.2.134 --fast
