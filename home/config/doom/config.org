...
DejaVu Serif is only installed in its regular weight - asking for ~:weight
'light~ makes every frame-wide font resize (~doom/increase-font-size~ and
friends) fail with "No fonts match", so the weight stays unspecified.
...
org-autolist.el (unmaintained since 2022) advises ~org-return~ with a quoted ~condition-case~ handler (~('error ...)~). Doom byte-compiles that advice when lsp-mode probes org at startup, so Emacs 32's byte-compiler prints a deprecation lint to the daemon's stderr. The lint is silenced in ~init.el~ by dropping that one ~byte-compile-warn-x~ message, since it fires during byte-compilation of the advice (not native compilation, so a deny-list does not help).
...
*** Drop org-lint's invalid-id-link checker

The table above only helps when *following* a link. Displaying a note is a
separate story: flycheck lints every org buffer it shows, and org-lint's
=invalid-id-link= checker force-rescans on the first =id:= link it sees, without
first checking whether that id already resolves:

#+begin_quote
: (unless id-locations-updated
:   (org-id-update-id-locations nil t)   ; force
:   (setq id-locations-updated t))
#+end_quote

=org-id-update-id-locations= then walks every agenda file, its archives, and
every org file open in Emacs, =file-truename='s and dedups the lot, and greps
each one for IDs. Measured here: 43s, independent of the note's size, on the
~half of my roam notes that contain an =id:= link. Warm switches between
already-linted buffers are 3-8ms, so this is the entire perceived lag when
moving between notes.

Removing the checker costs only "Unknown ID" warnings and leaves every other
org-lint check in place.
...
*** Web Dev (JS/TS/CSS)
**** Prisma
Features for the Prisma ORM configuration language
#+begin_src elisp :tangle packages.el
(package! prisma-mode
...
Agda is a dependently typed functional programming language. It is an extension of Martin-Löf’s type theory
and the latest in the tradition of languages developed in the programming logic group at Chalmers.

The Doom module is enabled as ~(agda +local)~ in [[file:init.el][init.el]], which loads ~agda2-mode~ from the Agda installation itself (via ~agda-mode locate~) instead of a separately pinned MELPA package. That way the elisp always matches the =agda= binary: nix puts ~agda.withPackages~ (including the standard library) on the doom-emacs wrapper's PATH, and the same package is installed via home-manager for shell use.

Agda's emacs mode only registers =.agda= and =.lagda= in ~auto-mode-alist~. For literate programming the other literate variants (markdown, org, reST, TeX, Typst, forester) need to be associated too; they type-check with ~C-c C-l~ just like a plain =.agda= file.
...
Pie is the little dependently typed language from /The Little Typer/, implemented as a Racket ~#lang~. It isn't in nixpkgs, so [[https://github.com/the-little-typer/pie][pie]] is built in-repo by ~packages/pie~ and ~modules.dev.pie~ puts the resulting pie-aware ~racket~ wrapper on PATH — racket-mode's default ~racket-program~ ("racket") resolves to it, so ~racket-run~ (=C-c C-c=) drops a =.pie= buffer straight into a Pie REPL. The Doom module is enabled as ~(racket +xp)~ in [[file:init.el][init.el]]; racket-mode doesn't know the =.pie= extension, so associate it here.

#+begin_src elisp
(add-to-list 'auto-mode-alist '("\\.pie\\'" . racket-mode))
#+end_src

Org-Babel can evaluate Pie too: a =#+begin_src pie= block is written to a temporary =#lang pie= file and run through the same pie-aware ~racket~, so the normalized results — or the type checker's complaint — come back inline. There is no upstream ~ob-pie~, so this is a small hand-rolled backend; =C-c '= edits blocks in racket-mode, and a block that already opens with its own ~#lang~ line is run as-is. Registering ~(pie . t)~ keeps Doom's lazy loader from trying to ~require~ a nonexistent ~ob-pie~.
...
To alleviate, it should only check the syntax on file-save (not on every idle
keystroke). The previous ~flycheck-syntax-automatically~ setq was a no-op: the
real variable is ~flycheck-check-syntax-automatically~.

Additionally, the flycheck build ships a built-in ~org-lint~ checker that runs a
full org-element parse on every check, which makes editing org files laggy - so
...
Automatically start LSP on file open, guess root. Some prog-mode derivatives have no LSP server and their own interaction machinery instead (Agda), so those are excluded rather than letting lsp-mode warn about a missing languageId on every file visit.
#+begin_src elisp
...
Nothing to do about puppeteer here: mmdc renders through it, and on macOS it looks for a
pinned Chrome build that nix never downloads (nixpkgs has no chromium on darwin), so every
block used to fail with "Could not find Chrome". The mmdc in =home/modules/tools/mermaid.nix=
is wrapped with =PUPPETEER_EXECUTABLE_PATH= pointing at the system Chrome, which fixes it
for every caller rather than just for org babel.
...
Hide Modeline in Treemacs. Doom v2.1.0 (pinned by nix-doom-emacs-unstraightened)
doesn't ship hide-mode-line as a core package; declare it explicitly so
~hide-mode-line-mode~ is defined when ~treemacs-mode-hook~ fires. Without this,
the hook errors mid-treemacs-mode init and leaves the buffer half-initialised
(in-this-buffer set, projects-end nil), which crashes every post-command.
...
bit - but it draws a thin overline across the top of the modeline, which I
don't want, so it stays disabled.
...
Guard with ~fboundp~ in case ~hide-mode-line~ isn't available (see the earlier
section under Doom Modeline for the full explanation).
...
Keep Doom's incrementally deferred packages lazy.  Daemon sessions otherwise
load the whole queue synchronously before becoming ready, which adds roughly
five seconds here.
#+begin_src elisp
...
*** Babel evaluation via Claude Code

ob-gptel talks to raw model APIs, but I also want to drive Claude Code from org source blocks: it runs on my subscription (with Fable as the default model), and it brings agentic tools with it, so a block can read or even modify a repo instead of just answering from context. There is no existing babel backend for the Claude CLI, so this is a small hand-rolled one. It pipes the block body to ~claude -p~ and asynchronously replaces a placeholder with the reply, the same pattern ob-gptel uses.

Supported header arguments:
- ~:dir~ sets the working directory, pointing Claude at a repo (handled by org-babel itself).
- ~:model~ passes ~--model~; without it, the CLI default applies.
- ~:session~ threads a conversation across blocks. The CLI mints a new session id on every ~-p~ run, so the JSON output's ~session_id~ is stored under the session name and passed via ~--resume~ on the next block.
- ~:tools~ passes ~--allowedTools~ (e.g. ~"Bash(git:*),Edit"~).
- ~:permission-mode~ passes ~--permission-mode~ (e.g. ~acceptEdits~ for blocks that should edit files).
- ~:format~ is ~org~ by default, converting the markdown reply via gptel's converter; set it to ~markdown~ to keep the raw text.

Agentic blocks can run for a while; a pending block can be cancelled by killing the ~ *ob-claude-code*~ buffer's process. For multi-line prose answers, ~:results raw~ inserts more nicely than the default fixed-width block.
...
Sync, credentials and the account definitions live in =home/modules/mail=. This
section only wires mu4e to the maildir that mbsync writes to.

The ~+gmail~ flag decides per message whether Gmail semantics apply by matching
~gmail~ against the root maildir, but it picks the /Sent/ behaviour from the
envelope address, which never matches ~googlemail.com~. Listing the account
...
One context per account, switched automatically by the maildir a message lives
in. ~[Google Mail]/All Mail~ is deliberately not synced - it holds a copy of every
message under every label - so archiving a Gmail message only drops its INBOX
label rather than fetching it back.
