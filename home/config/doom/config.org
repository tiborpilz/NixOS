...
**** Manual Triggering
I want to be able to trigger company manually, so I'm binding it to ~C-n~ - but
since that is already bound to evil-paste-pop, I have to first unbind it.
...
**** Gitlab
Although Magit/Forge supports Gitlab, there is some functionality missing,
especially in regards to the pipelines. For that, I'm using ~lab.el~

#+begin_src elisp :tangle packages.el
(package! lab :recipe (:host github :repo "isamert/lab.el"))
#+end_src

Wait for the package to be loaded before setting the variables.
#+begin_src elisp
(use-package lab
  :after magit
  :config
...
#+end_src
** Eshell
Eshell is a shell implemented in Emacs Lisp. It is a replacement for command
shells such as bash, zsh or fish, providing a general purpose command line
environment, which also supports Emacs features like command completion,
history, and redirection.

*** Prompt
I start with generating a macro which accepts a name, an icon, a form or
variable, and potentially face properties. This macro generates anonymous
functions that are merged and assessed when prompted.

(Shoutout to [[https://www.modernemacs.com/post/custom-eshell][Modern Emacs]])
...
The seperator between the sections
#+begin_src elisp
(setq esh-sep " ")
#+end_src

The Seperator between a section icon and form
#+begin_src elisp
(setq esh-section-delim " ")
#+end_src

The Eshell prompt header
#+begin_src elisp
(setq esh-header "\n")
#+end_src

The prompt regexp and string, since I don't want to vary them based on my login,
...
Now, I can generate some sections.
...
Now, I can generate the prompt function. First, I define the list of sections
#+begin_src elisp
(setq eshell-funcs (list esh-dir esh-git esh-python esh-clock))
#+end_src

Then, I generate the prompt function
#+begin_src elisp
