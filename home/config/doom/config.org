...
For all monospoace/code buffers, I'm going with a low, but non-zero value.
#+begin_src elisp
...
**** Top padding
Using ~header-line~ to be empty, I can fake top padding.
...
Additionally, I'm adding a helper function to setup gaps on both sides of the
buffer.
#+begin_src elisp
(defun clear-sides ()
...
I'm using ~clear-sides~ in ~org-mode~ and some others to start.
...
*** Dhall
Dhall is a configuration language, a bit like Jsonnet or Nix, but without the
turing completeness.

First, a package for a major mode.
#+begin_src elisp :tangle packages.el
...
#+end_src
*** TODO Go LSP
The LSP is misbehaving, so I'll have to set it up manually.
** Tools
*** Code Completion
