...
Adds a nixos-options backends list.
...
Since ~nvm--find-exact-version-for "16" is not available from the start, I'm
setting the copilot-node-executable in a callback hook, after using nvm-use.
...
**** LSP-Bridge
As an alternative to ~lsp-mode~, which can be quite sluggish, I'm using
~lsp-bridge~.
