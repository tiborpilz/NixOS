...
org-autolist.el (unmaintained since 2022) uses a quoted ~condition-case~ handler (~('error ...)~), which the byte compiler in Emacs 32 warns about whenever the file gets JIT native-compiled, i.e. on the first start after every rebuild. Denying native compilation for it silences the warning; the byte-compiled version is plenty for a package this size. The deny-list entry has to be set before the package is loaded, hence outside ~:config~.
