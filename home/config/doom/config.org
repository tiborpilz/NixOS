...
Rather than relying on the traditional agenda view that becomes sluggish with many files, we use ~org-ql~ to create fast, database-driven task queries. This approach scales better and provides more flexibility for complex task organization.

**** Database Refresh Strategy

Since you mentioned files created after Emacs starts don't show up in agenda, we ensure the database stays current by refreshing it automatically and providing manual refresh commands.
