...
org-autolist.el (unmaintained since 2022) uses a quoted ~condition-case~ handler (~('error ...)~), which the byte compiler in Emacs 32 warns about whenever the file gets JIT native-compiled, i.e. on the first start after every rebuild. The same applies to evil-escape.el, which Doom's evil module pulls in and loads right at startup - that one is the warning that shows up on launch. Denying native compilation for both silences the warnings; the byte-compiled versions are plenty for packages this size. The deny-list entries have to be set before the packages are loaded, hence outside ~:config~.
