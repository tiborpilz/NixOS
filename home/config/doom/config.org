...
Mermaid renders node labels into an SVG ~<foreignObject>~ by default, which librsvg - and
therefore emacs' svg rendering - doesn't implement, so the boxes come out empty. The config
file (deployed by =home/modules/editors/emacs.nix=) sets ~htmlLabels: false~, which makes mmdc
emit plain ~<text>~/~<tspan>~ nodes instead. See [[https://github.com/mermaid-js/mermaid-cli/issues/112][mermaid-cli#112]].
