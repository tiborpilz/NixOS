...
;; Drop Logseq's backup/recycle dirs - they hold timestamped copies of live
;; notes (same :ID:) and only bloat the agenda / org-ql / org-id scans.
(setq org-agenda-files
      (seq-remove
       (lambda (f) (string-match-p "/logseq/" f))
       (append
...
;; Keep Logseq's backup/recycle/version dirs out of the org-roam DB. Each backup
;; carries the same :ID: as its live note, so indexing them creates stale
;; duplicate nodes that can win the id and make links resolve to a backup copy.
;; NB: org-roam matches this against the path *relative to* org-roam-directory,
;; so the patterns must not start with a slash.
(after! org-roam
  (setq org-roam-file-exclude-regexp
        (append (ensure-list org-roam-file-exclude-regexp)
                '("logseq/bak/" "logseq/\\.recycle/" "logseq/version-files/"))))

;; Following an id: link whose target isn't already in `org-id-locations' makes
;; org fall back to `org-id-update-id-locations', a synchronous scan of every
;; agenda file (~600 here, 30s+ freeze). Populate the table straight from the
;; org-roam DB instead (~20ms), so roam links always resolve without a scan.
(defun my/org-id-locations-from-roam-db (&rest _)
  "Populate `org-id-locations' from the org-roam DB."
...
(advice-add 'org-roam-db-sync :after #'my/org-id-locations-from-roam-db)
