...
Animated recordings of the terminal setup, captured from a real Kitty window (so Fira Code ligatures and Nerd Font glyphs render exactly as configured). They are regenerated in CI by the [Screenshots workflow](.github/workflows/screenshots.yml); see [`screenshots/`](screenshots/) for the scene scripts. More scenes live on the [overview page](https://tiborpilz.github.io/NixOS/).

| Telescope navigation | tmux session |
| --- | --- |
| ![Neovim Telescope recording](screenshots/output/neovim-telescope.gif) | ![tmux recording](screenshots/output/tmux.gif) |
